...
test_that("summary.pd_unit() computes the peritonitis rate and PF from the tibbles", {
  x <- make_unit(
    tpyar = 2,
    patients = tibble::tibble(patient_id = "ABC1234", n_episodes = 2L),
    infections = tibble::tibble(
      patient_id = c("ABC1234", "ABC1234"),
      catheter_id = c("ABC1234_01", "ABC1234_01"),
      episode_type = c(NA_character_, "relapsing"),
      counts_toward_rate = c(TRUE, FALSE)
    )
  )
  s <- summary(x)
  expect_equal(s$rate_num, 1)
  expect_equal(s$rate_den, 2)
  expect_equal(s$rate, 0.5)
  expect_false(s$rate_met)
  expect_equal(s$pf_num, 0)
  expect_equal(s$pf_den, 1)
  expect_equal(s$pf, 0)
  expect_false(s$pf_met)
  expect_equal(as.integer(s$episode_types[["uncategorised"]]), 1)
  expect_equal(as.integer(s$episode_types[["relapsing"]]), 1)
  expect_equal(unname(s$cohort["incident"]), 0L)
  expect_equal(unname(s$cohort["prevalent"]), 1L)
})

test_that("summary.pd_unit() reports MET when indicators clear the ISPD benchmarks", {
  x <- make_unit(patients = tibble::tibble(patient_id = "ABC1234", n_episodes = 0L))
  s <- summary(x)
  expect_equal(s$rate, 0)
  expect_true(s$rate_met)
  expect_equal(s$pf, 1)
  expect_true(s$pf_met)
})

test_that("summary.pd_unit() prints a labelled report and returns invisibly", {
  x <- make_unit()
  expect_output(summary(x), "Peritonitis rate")
  expect_output(summary(x), "ISPD benchmark")
  expect_output(summary(x), "Episodes by type")
  expect_output(summary(x), "Cohort")
  expect_invisible(summary(x))
})

test_that("summary.pd_unit() avoids division by zero for an empty unit", {
  x <- new_pd_unit(unit_id = "Empty PD Unit", t0 = T0, t1 = T1,
                   n_new = 0L, n_patients = 0L, tpyar = 0)
  s <- summary(x)
  expect_true(is.na(s$rate))
  expect_true(is.na(s$pf))
  expect_length(s$episode_types, 0)
})
